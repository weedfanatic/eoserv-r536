...
class Guild_Member
{
	public:
		...
		Guild_Member(std::string name_, int rank_ = 10, std::string rank_string_ = "")
			: name(name_)
			, rank(std::max(0, std::min(10, rank_)))
...
class Guild_Create : public std::enable_shared_from_this<Guild_Create>
{
	public:
		...
		void AddMember(std::string character, int rank = 10);
...
class Guild : public std::enable_shared_from_this<Guild>
{
	public:
		...
		void AddMember(Character *joined, Character *recruiter, bool alert = false, int rank = 10);
